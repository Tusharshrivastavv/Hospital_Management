  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        ...
        let isSlotAvailable = docInfo.slots_booked[slotDate]?.includes(formattedTime) ? false : true;
    ...
    <div>
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
          ...
          <p className="text-gray-500 font-medium mt-4">Fee: <span className='text-gray-600 font-bold'>{currencySymbol}{docInfo.fees}</span></p>
        </div>
      </div>
      <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
        ...
        <button onClick={bookAppointment} className="bg-primary text-white text-sm font-light px-14 py-3 rounded-full my-6">Confirm Booking</button>
      </div>
